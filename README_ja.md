...
1. **pywinauto UIA** — 新しいExplorerウィンドウを開き、UI Automationで接続。Ctrl+Tで新しいタブを作成し、UIA ValuePatternでアドレスバーにパスを直接設定。入力後にパスを検証し、失敗時は自動リトライ。最も信頼性が高い方式。
...
> **⚠️ 注意（ctypes SendInput フォールバック）:** タブを開いている最中はキーボードやマウスを操作しないでください。ctypes フォールバック方式はOSレベルのキーストローク送信（`SendInput`）を使用しているため、操作中の入力が自動化処理に干渉する可能性があります。pywinauto UIA 方式はターゲット指定のUIオートメーション（グローバルキーストロークなし）を使用するため、ユーザーの入力に影響されません。
